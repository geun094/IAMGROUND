...
public class SecurityConfig {
	...
	public WebSecurityCustomizer webSecurityCustomizer() {
		return (web) -> web.ignoring()
				           .antMatchers("/css/**", "/js/**", "/images/**", "/lib/**", "/regist/**");
	}
...
//	    		.antMatchers("/").authenticated()								// "/"요청은 인증되어야 됨(로그인 페이지로 안내)
	    		...
	    		.antMatchers(HttpMethod.POST, "/menu/**").hasRole("PROMEMBER")
	    		.antMatchers(HttpMethod.POST, "/menu/**").hasRole("ADMIN")
	    		
	    		.antMatchers(HttpMethod.GET, "/mypage/mypage-mem").hasRole("MEMBER")
	    		.antMatchers(HttpMethod.POST, "/mypage/mypage-mem-update").hasRole("MEMBER")
	    		
	    		.antMatchers(HttpMethod.GET, "/mypage/mypage-promem-before").hasRole("MEMBER")
	    		.antMatchers(HttpMethod.POST, "/mypage/mypage-promem-before-update").hasRole("MEMBER")
	    		
	    		.antMatchers(HttpMethod.GET, "/mypage/mypage-promem-after").hasRole("PROMEMBER")
	    		.antMatchers(HttpMethod.POST, "/mypage/mypage-promem-after-update").hasRole("PROMEMBER")
	    		
	    		.antMatchers("/admin/**").hasRole("ADMIN")
				.antMatchers("/order/**").hasRole("PROMEMBER")
		    ...
		    /* 로그인 페이지 및 로그인 성공 후 포워딩 경로 설정 */
		    	.and()
	             ...
	             .loginPage("/login/login")                              // login Page로써의 의미이자 해당 login Page에서 submit 요청하는 경로의 의미를 지닌다.(부연 설명 고려)
	             .successForwardUrl("/")                                 // 로그인 성공 시 가게 될 페이지
	             .failureForwardUrl("/login/loginError")										// 로그인 성공 시 가게 될 페이지
			...
			.and()
			    ...
			    .accessDeniedPage("/")									// 접근 거부 시 경로 설정
